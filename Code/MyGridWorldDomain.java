 ...
 *
 */
public class MyGridWorldDomain implements DomainGenerator {
     ...
     */
    public MyGridWorldDomain(int width, int height){
        this.width = width;
        this.height = height;
     ...
     */
    public MyGridWorldDomain(int [][] map){
        this.setMap(map);
        this.setDeterministicTransitionDynamics();
    }
    ...
    public static State getOneAgentNoLocationState(Domain d, int ax, int ay){
        ...
        s.addObject(new MutableObjectInstance(d.getObjectClass(CLASSAGENT), CLASSAGENT+0));
        MyGridWorldDomain.setAgent(s, ax, ay);
     ...
     */
    public static MyValueFunctionVisualizerGUI getGridWorldValueFunctionVisualization(List <State> states, ValueFunction valueFunction, Policy p){
        return MyValueFunctionVisualizerGUI.createGridWorldBasedValueFunctionVisualizerGUI(states, valueFunction, p,
    ...
    public class MovementAction extends SimpleAction implements FullActionModel{
        ...
        protected State performActionHelper(State s, GroundedAction groundedAction) {
            ...
            int [] dcomps = MyGridWorldDomain.this.movementDirectionFromIndex(dir);
            return MyGridWorldDomain.this.move(s, dcomps[0], dcomps[1], this.map);
        }
        ...
        public List<TransitionProbability> getTransitions(State s, GroundedAction groundedAction) {
            ...
            for(int i = 0; i < directionProbs.length; i++){
                ...
                State ns = s.copy();
                int [] dcomps = MyGridWorldDomain.this.movementDirectionFromIndex(i);
                ns = MyGridWorldDomain.this.move(ns, dcomps[0], dcomps[1], this.map);
    ...
    public class WallToPF extends PropositionalFunction{
        ...
        public WallToPF(String name, Domain domain, String[] parameterClasses, int direction) {
            super(name, domain, parameterClasses);
            int [] dcomps = MyGridWorldDomain.this.movementDirectionFromIndex(direction);
            xdelta = dcomps[0];
            ydelta = dcomps[1];
        }
        ...
        public boolean isTrue(State st, String... params) {
            ...
            if(cx < 0 || cx >= MyGridWorldDomain.this.width || cy < 0 || cy >= MyGridWorldDomain.this.height || MyGridWorldDomain.this.map[cx][cy] == 1 ||
                    (xdelta > 0 && (MyGridWorldDomain.this.map[ax][ay] == 3 || MyGridWorldDomain.this.map[ax][ay] == 4)) || (xdelta < 0 && (MyGridWorldDomain.this.map[cx][cy] == 3 || MyGridWorldDomain.this.map[cx][cy] == 4)) ||
                    (ydelta > 0 && (MyGridWorldDomain.this.map[ax][ay] == 2 || MyGridWorldDomain.this.map[ax][ay] == 4)) || (ydelta < 0 && (MyGridWorldDomain.this.map[cx][cy] == 2 || MyGridWorldDomain.this.map[cx][cy] == 4)) ){
                return true;
            }
    ...
    public static void main(String[] args) {
        ...
        MyGridWorldDomain gwdg = new MyGridWorldDomain(11, 11);
